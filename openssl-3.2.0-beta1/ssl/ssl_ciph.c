...
static const ssl_cipher_table ssl_cipher_table_cipher[SSL_ENC_NUM_IDX] = {
    ...
    {SSL_ASCON80PQ, NID_ascon_80PQ},
    {SSL_ASCON128, NID_ascon_128},
    {SSL_ASCON128A, NID_ascon_128A}};
...
static const SSL_CIPHER cipher_aliases[] = {
    ...
    {0, SSL_TXT_ASCON80PQ, NULL, 0, 0, 0, SSL_ASCON},
    {0, SSL_TXT_ASCON128, NULL, 0, 0, 0, SSL_ASCON},
    {0, SSL_TXT_ASCON128A, NULL, 0, 0, 0, SSL_ASCON},
...
int ssl_load_ciphers(SSL_CTX *ctx)
{
    ...
    for (i = 0, t = ssl_cipher_table_cipher; i < SSL_ENC_NUM_IDX; i++, t++)
    {
        if (t->nid != NID_undef)
        {
            ...
            if (cipher == NULL)
            {
                printf("number %d - did not find any cipher\n", i);
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            while (ca_list[j])
            {
                ...
                else if (ca_list[j]->stdname != NULL && strncmp(buf, ca_list[j]->stdname, buflen) == 0 && ca_list[j]->stdname[buflen] == '\0')
                {
                    found = 1;
                    break;
                }
                              ...
                              STACK_OF(SSL_CIPHER) * tls13_ciphersuites)
{
    ...
    for (i = sk_SSL_CIPHER_num(tls13_ciphersuites) - 1; i >= 0; i--)
    {
        ...
        if ((sslc->algorithm_enc & ctx->disabled_enc_mask) == 0 && (ssl_cipher_table_mac[sslc->algorithm2 & SSL_HANDSHAKE_MAC_MASK].mask & ctx->disabled_mac_mask) == 0)
        {
