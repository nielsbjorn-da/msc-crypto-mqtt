...
for ((i=1; i<=$num_repetitions; i++)); do
    ...
    for replacement_value in "${shuffled_algorithms[@]}"; do
        ...
        echo "$groups_line"
        client/mosquitto_sub -t "test/topic" -h 185.187.145.40 -p 8883 --cafile "certs/$sig_alg_value/ca.crt" --cert "certs/$sig_alg_value/client.crt" --key "certs/$sig_alg_value/client.key" &
        mosquitto_sub_pid=$!
        sleep 2
    done
