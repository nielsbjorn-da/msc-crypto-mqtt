...
#        decode_pk = []
        ...
        for line in lines:
            ...
            match2 = re.search(r'Decode sig (Dilithium2|Dilithium3|Dilithium5|Falcon-512|Falcon-1024) execution time: (\d+) micro seconds.', line)
#            match3 = re.search(r'Decode PK (Dilithium|Falcon) execution time: (\d+) micro seconds.', line)
            match4 = re.search(r'Verification (Dilithium2|Dilithium3|Dilithium5|Falcon-512|Falcon-1024) execution time: (\d+) micro seconds.', line)
...
#        average_decode_pk = np.mean(decode_pk)
        ...
        data[algorithm_name  + "-Decode signature & PK"] = average_decode_sig
...
plt.title('Phases subscriber - Design B')
